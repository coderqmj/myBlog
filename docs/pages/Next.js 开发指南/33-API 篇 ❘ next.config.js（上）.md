...
其中 `phase` 表示配置加载的当前上下文。通过[查看源码](https://github.com/vercel/next.js/blob/5e6b008b561caf2710ab7be63320a3d549474a5b/packages/next/shared/lib/constants.ts#L19-L23 "https://github.com/vercel/next.js/blob/5e6b008b561caf2710ab7be63320a3d549474a5b/packages/next/shared/lib/constants.ts#L19-L23")，可以知道 `phase` 的值一共有 5 个：
...
在这个例子中，注释行的地方就是你可以放配置的地方，实际上，Next.js 定义的配置非常多，可以查看[源码配置文件](https://github.com/vercel/next.js/blob/canary/packages/next/src/server/config-shared.ts "https://github.com/vercel/next.js/blob/canary/packages/next/src/server/config-shared.ts")。
...
![截屏2023-11-09 下午3.40.56.png](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/ca0818d69f76450182e9e9834080e0eb~tplv-k3u1fbpfcp-jj-mark:1600:0:0:0:q75.jpg#?w=2304&h=1528&s=400155&e=png&b=2a2a2a)
...
![截屏2023-11-09 下午4.00.05.png](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/f430b2c5e9a44a7d88dcaf32a3bdbad9~tplv-k3u1fbpfcp-jj-mark:1600:0:0:0:q75.jpg#?w=2294&h=1516&s=410172&e=png&b=2a2a2a)
...
![image.png](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/72fc974d27674c02a4e40057c49cc1ac~tplv-k3u1fbpfcp-jj-mark:1600:0:0:0:q75.jpg#?w=786&h=84&s=20629&e=png&b=282828)

访问 `/blog/hello-world` 也是有的：

![image.png](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/0c5756cc8262461a945a66436db9ee92~tplv-k3u1fbpfcp-jj-mark:1600:0:0:0:q75.jpg#?w=700&h=88&s=17996&e=png&b=282828)
...
![image.png](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/0738859e301945d9967e538636722a43~tplv-k3u1fbpfcp-jj-mark:1600:0:0:0:q75.jpg#?w=402&h=52&s=7281&e=png&b=282828)
...
![image.png](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/ea5a1ecfec794a40b85c6684899f2914~tplv-k3u1fbpfcp-jj-mark:1600:0:0:0:q75.jpg#?w=248&h=128&s=11341&e=png&b=282828)
...
![image.png](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/f286f2e743624f39b7f0ab7678bc7a10~tplv-k3u1fbpfcp-jj-mark:1600:0:0:0:q75.jpg#?w=1548&h=544&s=121143&e=png&b=282828)

cookie 指的是其中的 Cookie 标头，Next.js 已经自动做了解析，所以可以直接判断 Cookie 中的字段值：

![image (1).png](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/1b0f0c3bde944941b01ece1cd01f43c0~tplv-k3u1fbpfcp-jj-mark:1600:0:0:0:q75.jpg#?w=1548&h=544&s=118810&e=png&b=282828)
...
[X-DNS-Prefetch-Control](https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Headers/X-DNS-Prefetch-Control "https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Headers/X-DNS-Prefetch-Control") 头控制着浏览器的 DNS 预读取功能。DNS 预读取是一项使浏览器主动去执行域名解析的功能，其范围包括文档的所有链接，无论是图片的，CSS 的，还是 JavaScript 等其他用户能够点击的 URL。
...
[Strict-Transport-Security](https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Headers/Strict-Transport-Security "https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Headers/Strict-Transport-Security")（通常简称为 HSTS）响应标头用来通知浏览器应该只通过 HTTPS 访问该站点，并且以后使用 HTTP 访问该站点的所有尝试都应自动重定向到 HTTPS。
...
[X-Frame-Options](https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Headers/X-Frame-Options "https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Headers/X-Frame-Options") HTTP 响应头是用来给浏览器指示允许一个页面可否在 `<frame>`、`<iframe>`、`<embed>` 或者 `<object>` 中展现的标记。站点可以通过确保网站没有被嵌入到别人的站点里面，从而避免点击劫持 (en-US)攻击。

此标头已经被 [frame-ancestors](https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Headers/Content-Security-Policy/frame-ancestors "https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Headers/Content-Security-Policy/frame-ancestors") 替代，它在现代浏览器中有更好的支持。

#### Permissions-Policy

[Permissions-Policy](https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Headers/Permissions-Policy "https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Headers/Permissions-Policy") 响应标头提供了一种可以在本页面或包含的 iframe 上启用或禁止浏览器特性的机制，之前叫做 `Feature-Policy`。
...
如果 `Content-Type` 标头没有被显示设置，[X-Content-Type-Options](https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Headers/X-Content-Type-Options "https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Headers/X-Content-Type-Options") 会阻止浏览器尝试猜测内容类型。这可以防止允许用户上传和共享文件的网站受到 XSS 攻击。
...
[Referrer-Policy](https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Headers/Referrer-Policy "https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Headers/Referrer-Policy") 控制当从当前网页导航到另一个网页时携带的信息内容：
...
![image.png](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/cf100c8b0fbc475b9a2472359b7e3190~tplv-k3u1fbpfcp-jj-mark:1600:0:0:0:q75.jpg#?w=900&h=36&s=49552&e=png&b=010509)
...
![image.png](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/4cef281c1d70417b9104f5ce57478ecd~tplv-k3u1fbpfcp-jj-mark:1600:0:0:0:q75.jpg#?w=602&h=48&s=41058&e=png&b=010508)
...
![image.png](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/6fceaf76a61f403b81e8cf524e622c9b~tplv-k3u1fbpfcp-jj-mark:1600:0:0:0:q75.jpg#?w=888&h=44&s=56783&e=png&b=010508)
...
1. [nextjs.org/docs/app/ap…](https://nextjs.org/docs/app/api-reference/next-config-js "https://nextjs.org/docs/app/api-reference/next-config-js")
2. [nextjs.org/docs/app/ap…](https://nextjs.org/docs/app/api-reference/next-config-js/headers "https://nextjs.org/docs/app/api-reference/next-config-js/headers")
3. [nextjs.org/docs/app/ap…](https://nextjs.org/docs/app/api-reference/next-config-js/redirects "https://nextjs.org/docs/app/api-reference/next-config-js/redirects")
4. [nextjs.org/docs/app/ap…](https://nextjs.org/docs/app/api-reference/next-config-js/rewrites "https://nextjs.org/docs/app/api-reference/next-config-js/rewrites")