...
```
1.一开始进来的时候，先要先用我提供的app.routes(productKey,{'path': component})生成routeMap，然后拿着routeMap和productKey去注册模块
2.首先进来需要加载模块，看是否有ProductKey，如果没有就展示概览页，有的话就展示对应运营端应用
3.如果之前是有模块的，并且不是当前模块，需要清理模块
