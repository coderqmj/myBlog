...
协商缓存：
	...
	区别：
		...
		Last-Modified：适用于不频繁修改的文件，比如图片资源，第三方依赖，只需要进行轻量验证即可，无需计算哈希值
		Etag：适用于频繁更改的文件，比如我们的js资源版本经常更新的
```
...
5.HTTP3.0
	...
	3.TCP无法更新，只能绕过发明TCP和UDP之外的传输协议
		...
		2.UDP传输没有可靠性，但是QUIC在UDP的基础上增加了一层，保证了数据传输的可靠性。提供了数据包重传，拥塞控制台等一些TCP特性
...
为什么CDN不受跨域影响：
	0.<script>、<img>、<iframe>、<link>本身就是有跨域功能的
	1.保证了web的开放性和扩展性吧
	2.CDN用于加载React.js，Vue.js库，如果限制的话，不利于分发和缓存
```
...
```
同源策略：
1.同源策略是浏览器最核心的安全功能，没有同源策略，网页很容易受到xss，csrf攻击。
2.同源指的是同域名、同端口、同协议。三者必须同时满足，即使是两个不同域名执行同一个ip地址，也是非同源。

跨域方案：
1.中间层代理：
	- 同源策略是浏览器的，但是服务端没有这个策略，可以用中间层服务器做请求转发
2.webpack配置代理
	- proxy: {
...
|     形式     |                         字符串                         | 对象               |                          |                |
|     应用     |                      登录、购物车                      | 登录、购物车       |                          |                |
|     跨域     |        支持跨域（设置domain，比如.example.com）        | 不支持跨域         |                          |                |
...
一、XSS：跨站脚本攻击
	...
	3.怎么攻击：
		1.存储型XSS攻击：恶意脚本存储在有漏洞的服务器，用户访问，脚本获取信息，发送给黑客的服务器；
			1.例子：喜马拉雅电台，用户创建电台的时候，专辑名称是一个script标签
...
二、CSRF——跨站请求伪造
	...
	3.攻击方式：
		...
		3.引诱点击链接：比如QQ群里点击下载什么美女图片啊，里面是有<a></a>标签，点进去就发起请求了，点了之后实际上就调用了转账接口，完成转账；
	4.如何防御：
		...
		3.CSEF Token
			...
			3.如果是第三方，那肯定是没有Token的，因为Token在页面中，必须要打开页面；
			4.<input type="hidden" name="csrf-token" value="w32fvsd325g"  />
```
