...
3.HTTP1.1
	...
	5.带宽竞争，有可能先请求了不重要的资源，而重要的资源再后面请求到
...
### 16如果发起一个请求，又发起了一个请求，但是第一个请求的结果在第二个请求之后到达
...
### 27.WebSocket 如何连接？服务端给客户端发送的http状态码是什么？
...
```
大致流程：
  1.浏览器获取HTML文件，然后渲染进程对文件进行解析转换，转换浏览器可以理解的DOM Tree
  2.与此同时，渲染引擎将css样式表进行解析，生成浏览器可以理解的StyleSheets，然后还需要转换样式表中的属性值（比如2em、blue、bold，渲染引擎理解不了，需要转rgb，转具体的值），使其标准化，再计算出每个DOM节点对应的样式（如何计算，见7）
  3.接着将DOM Tree与Style Rules合成为布局树Render Tree。
  4.接着进入布局（Layout）阶段，就是计算出DOM树中可见元素的在屏幕上的位置。
  	4.1 创建布局树：通过计算的样式和DOM树，生成可见元素的布局树，布局树会忽略display：none这些不可见的元素
  	4.2 布局计算：带学
  5.有了布局树，并且每个元素的具体位置也计算出来了，还不能开始绘制图像，因为页面中有很多复杂的效果，#D转换，页面滚动，或者有z-index，为了方便实现这些效果，渲染引擎会给特定的节点生成专用的图层，生成图层树，各个图层叠加在一起才生成最终图像
