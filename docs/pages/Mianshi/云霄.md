...
```
云霄——云服务器管理系统
	作为云霄项目前端负责人，基于微前端的理念结合当前业务需求，手动实现了微前端，开发CLI工具、公共模块抽离成npm包（如通用的发起请求函数，路由注册，获取平台信息等通用方法），内置公共webpack，也实现了发布功能。使得前端团队可以专注于业务开发，无需关心其他配置，能高效开发项目，并有效维护现有20+管理系统
	1.技术栈：React16+,Redux,Redux-tool-kit,TeaDesign,Typescript,Axios,Webpack,Hooks
	2.手动实现了微前端，将子应用挂载到主应用上
	3.自定义webpack自动部署静态资源插件，可以快速发布到测试环境进行验证
	4.从0-1实现运营系统变更发布功能，实现了发布后机器人在群内进行发布周知功能
	5.封装Cli的NPM库，提供丰富的方法给到子应用，比如注册子应用方法，通用请求方法request，让子应用聚焦于业务开发，无需在各自项目里面配置
	6.封装公共方法解决请求重复，请求返回时序问题
	7.封装yunxiao-script的npm包给10+子应用使用，提供dev，build和发布的脚本，webpack配置均由npm包生成，子应用无需配置webpack，只需关注自身业务
	8.使用webpack对项目进行打包优化
...
2.微前端框架带有这种父子应用之间的通信（actions），我们项目中不涉及
...
5.然后又是有这么一个好机会，想去手动做一做对自己有提升的事情
6.微前端还涉及到应用之间的通信，我们不涉及
```
