...
- JS为什么需要引擎？
  - 编写的JS代码无论是在Node还是浏览器中运行，都需要被CPU执行
  - CPU只认识自己的指令集，实际上是机器语言，才能被CPU执行，识别不了JS这种高级语言
...
```
1.JS代码不管是在Node还是浏览器中运行，他最终都是交给CPU去执行的。
2.CPU它可不认识这些let const这些JS语法，它只认识自己的指令集，也就是机器语言
3.所以如果想要被运行的话，那就需要一种工具帮我们做翻译，那这个工具就叫JS引擎，比如V8引擎
4.
```

### V8引擎

- 谷歌开发，C++编写，用于Chorme和Node.js
- V8可以独立运行，也可以嵌入其他C++应用程序

### 语言类型

- 解释型语言
  - 比如JS，Java这种，各种解释语法
  - 因为需要先解析成AST，在将AST转成字节码，才能运行，所以效率相对较低
- 编译型语言
  - 比如C++
  - 可以直接编译成给CPU执行的代码，执行效率很高
